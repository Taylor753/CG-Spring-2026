...
*Note* Use `plot_mesh_with_normals` to set the shading in the viewer to use the normals `n` you computed.
...
*Note* use `plot_lines` to plot lines as
```python
p = pv.Plotter()
p.add_mesh(to_pyvista_mesh(v, f), show_edges=True, scalars=...)
